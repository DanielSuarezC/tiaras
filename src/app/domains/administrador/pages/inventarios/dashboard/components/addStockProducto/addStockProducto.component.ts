...
export class AddStockProductoComponent implements OnInit {
      ...
      addProducto() {
        let producto = this.ProductosOnInventario.find(i => i.producto.nombre === this.form.get('insumoSearch')?.value);
        ...
        const stock = Number(this.form.get('insumoStock')?.value);
        if (!stock) {
        ...
        if (this.productosSeleccionados.find(p => p.producto.producto.idProducto === producto.producto.idProducto)) {
          ...
          this.form.get('insumoSearch')?.reset();
          this.form.get('insumoStock')?.reset();
